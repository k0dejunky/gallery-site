...
<ul>
    ...
    <li><strong>Traffic</strong> (<code>/admin/traffic</code>) — generate custom links (a short <code>?c=code</code> plus optional UTM tags) to see where visiting and signup traffic comes from. Each link targets a configurable path (default <code>/signup</code>), tracks visits (deduped per link/day/visitor) and counts the signups it credited. Per-link detail shows a 30-day daily breakdown, the credited accounts and recent visits. Terminating a link expires it: it stops recording and stops being credited at signup; reactivating restores it.</li>
