...
function Editor() {
  ...
  const saveOverOrigRef = useRef(saveOverOriginal);
  useEffect(() => { projectRef.current = project; }, [project]);
  useEffect(() => { titleRef.current = title; }, [title]);
  useEffect(() => { versionRef.current = version; }, [version]);
  useEffect(() => { saveOverOrigRef.current = saveOverOriginal; }, [saveOverOriginal]);

  const save = useCallback(async (silent) => {
    const p = projectRef.current, t = titleRef.current, v = versionRef.current, soo = saveOverOrigRef.current;
    if (!silent) setStatus('Saving...');
    try {
      const body = { _token: config.token, title: t, version: String(v), project: JSON.stringify(p) };
      if (soo) body.save_over_original = '1';
      ...
      setVersion(d.version); savedSnapshot.current = { project: structuredClone(p), version: d.version, title: t, saveOverOriginal: soo };
  ...
  const cancel = useCallback(() => {
    ...
    setProject(structuredClone(snap.project)); setVersion(snap.version); setTitle(snap.title); setSaveOverOriginal(false);
    setCurrentTime(0); setPlaying(false); setSelection(null); setStatus('Reverted to original');
  }, []);

  useEffect(() => {
    if (skipNextSave.current) { skipNextSave.current = false; dirtyRef.current = false; return; }
    if (!dirtyRef.current) return;
    dirtyRef.current = false;
    clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => save(true), 900);
  ...
  const onStagePointerDown = useCallback((e) => {
    ...
    const move = (ev) => {
      ...
      if (existing) { Object.assign(existing, { x, y, w, h }); dirtyRef.current = true; setProject((c) => structuredClone(c)); }
            ...
            <div className={`nle-stage${tool === 'blur' ? ' tool-blur' : ''}`} ref={stageRef} onPointerDown={onStagePointerDown}>
              <div className="nle-stage-inner" style={{ filter: cssFilter(project.filters), transform: cssCrop(crop) }}>
                <video ref={videoRef} src={config.sourceUrl} preload="metadata"
                  onLoadedMetadata={(e) => { const v = e.currentTarget.duration; setDuration(v); if (!clip.end) { setProject((c) => { const n = structuredClone(c); n.tracks.find((t) => t.type === 'video').clips[0].end = Number(v.toFixed(2)); return n; }); } }}
