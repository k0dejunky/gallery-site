...
class AutoPostQueue
{
     ...
     * word-boundary aware so "nipples" also matches "nipple"). Delegates to
     * App\Core\AutoPostText (extracted text-composition helpers).
     */
    public static function containsBannedWord(string $text): bool
    {
        return AutoPostText::containsBannedWord($text);
    }
    ...
    {
        return AutoPostText::stripBannedWords($text);
    }
    ...
    {
        return AutoPostText::templateSettings($platform);
    }
    ...
    /**
     * Refill an idle platform queue: when a platform has nothing queued,
     * schedule fresh posts so X / Reddit stay active. Galleries are sourced
     * fresh-first (never used for autopost) and then RECYCLED from previously
     * posted/skipped/failed ones so the refill always has content — an empty
    ...
    public static function scheduleRandomGalleriesWhenIdle(int $count = 24, int $hours = 24, ?string $platform = null): int
    {
        ...
        // Source galleries with media: never-used galleries first, then recycle
    ...
    public static function requeueFrom(int $sourceId, ?string $scheduledAt = null, ?string $text = null): int
    {
        ...
        $key = AutoPostText::normalizePlatform((string) $src['platform']);
