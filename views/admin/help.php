...
<ul>
    ...
    <li><strong>Plans</strong> — manage membership plans. Create, edit, toggle active/inactive, and delete plans. Plans have a name, price, billing cycle (monthly/yearly/lifetime), access level (1&ndash;3), description, and sort order. Inactive plans are dimmed in the table but still visible. Six plans are seeded by default: Silver ($5/mo, level 1), Gold ($10/mo, level 2), Platinum ($20/mo, level 3), Monthly ($9.99/mo, level 1), Yearly ($99.99/yr, level 1), Lifetime ($249.99, level 1).</li>
    ...
    <li><strong>Traffic</strong> (<code>/admin/traffic</code>) — generate custom links (a short <code>?c=code</code> plus optional UTM tags) to see where visiting and signup traffic comes from. Each link targets a configurable path (default <code>/signup</code>), tracks visits (deduped per link/day/visitor) and counts the signups it credited. Every share link is <strong>signed</strong> (<code>&amp;s=</code> HMAC), so visitors cannot forge traffic by inventing <code>?c=</code> codes — only links produced by the admin page are counted, and the stored cookie signature is re-verified at signup. Links have <strong>no time-based expiry</strong>: they keep recording and crediting until the admin clicks <strong>Terminate</strong> (expires it: visits stop and stale cookies are no longer credited) or <strong>Delete</strong> (removes it and its visit history).</li>
...
<ul>
    ...
    <li><strong>Traffic</strong> (admin only) — create traffic links, copy/share them, and read the visits/signups each link produced. Use <em>Detail</em> for the full breakdown and <em>Terminate</em> to expire a link.</li>
...
<ul>
    <li><strong>Plans</strong> — stored in the <code>plans</code> table with: name, slug, price, billing cycle (<code>monthly</code>, <code>yearly</code>, <code>lifetime</code>), access level (1&ndash;3), description, sort order, and active flag. The <code>level</code> field determines what content a plan grants access to: Silver = 1, Gold = 2, Platinum = 3.</li>
...
│   │   ├── TrafficController.php        Traffic links: create/terminate, visits & signup attribution
...
<ul>
    ...
    <li><strong>Admin</strong> — dashboard, gallery CRUD, photo management, photo/video editors, category CRUD, search, abandoned uploads, video projects/exports, trends, traffic links, promotion, audit log + rollback/purge, error logs.</li>
