...
<?php if (empty($processors)): ?>
    <div class="pay-empty">No payment processors configured yet. Use the form below to add one.</div>
<?php else: ?>
    <div class="pay-grid">
...
<div class="pay-form">
    ...
    <form method="post" action="<?= url('/admin/payment-processors') ?>">
        <?= csrf_field() ?>
        <div class="pay-form-grid">
            <div>
                ...
                <select name="provider" required>
                    ...
                    <?php foreach (\App\Models\PaymentProcessor::PROVIDERS as $prov): ?>
                        <option value="<?= e($prov) ?>"><?= e(\App\Models\PaymentProcessor::providerLabel($prov)) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                <label>Display name</label>
                <input type="text" name="name" placeholder="e.g. Stripe (card)" required>
            </div>
            <div>
                <label>Mode</label>
                <select name="mode">
            ...
            <div>
                ...
                <input type="text" name="api_key" placeholder="pk_...">
            </div>
            <div>
                <label>Secret key</label>
                <input type="password" name="secret_key" placeholder="sk_...">
            </div>
            <div>
            ...
            <div class="pay-check">
                <input type="checkbox" name="is_default" value="1" id="pd_default">
                <label for="pd_default">Default</label>
            </div>
            <div class="pay-check">
                <input type="checkbox" name="enabled" value="1" id="pd_enabled" checked>
                <label for="pd_enabled">Enabled</label>
            </div>
        </div>
        <p style="margin-top:1rem;">
            <button type="submit" class="btn">Save Processor</button>
        </p>
    </form>
</div>
