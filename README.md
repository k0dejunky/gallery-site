...
The script:
1. Installs `apache2`, `php` (+ extensions), `mysql-server`, `ffmpeg`
2. Enables Apache `rewrite` / `alias` modules
3. Copies the site code and creates an empty `storage/uploads`
4. Writes a global Apache `/gallery` alias (in `conf-available/gallery.conf`)
5. Creates the database + user, imports `schema.sql` (plans + admin seed)
6. Applies your admin email/password with a fresh bcrypt hash
7. Writes `.env`, sets `www-data` ownership/permissions
8. Restarts Apache and verifies the site responds
...
- **Upload limit:** `public/.htaccess` sets 10 GiB per file
