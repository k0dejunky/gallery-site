...
<header class="admin-thread-header">
    <div><a class="support-back" href="<?= url('/admin/support') ?>">&larr; Support tickets</a><h1>#<?= (int) $ticket['id'] ?> &middot; <?= e($ticket['subject']) ?></h1><p class="admin-ticket-meta"><span><?= e($ticket['email']) ?></span><span>Opened <?= e(tzdate('M j, Y g:i', $ticket['created_at'])) ?></span><span><?= count($replies) ?> repl<?= count($replies) === 1 ? 'y' : 'ies' ?></span></p></div>
...
<section class="support-conversation" aria-labelledby="admin-thread-heading">
    ...
    <article class="support-message original"><div class="support-message-meta"><strong>Original message</strong><small><?= e(tzdate('M j, Y g:i', $ticket['created_at'])) ?></small></div><div class="support-message-body"><?= nl2br(e($ticket['message'])) ?></div></article>
    <?php foreach ($replies as $reply): ?>
        <article class="support-message <?= $reply['author_role'] === 'admin' ? 'admin' : 'user-reply' ?>">
            <div class="support-message-meta"><strong><?= $reply['author_role'] === 'admin' ? 'Admin reply' : 'Customer reply' ?></strong><small><?= e(tzdate('M j, Y g:i', $reply['created_at'])) ?></small></div>
