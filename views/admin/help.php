...
<ul>
    <li><strong>Super Admin</strong> — unrestricted access, including role and permission management. Assigned to the admin account you set up during install.</li>
...
<ul>
    ...
    <li><strong>Media files</strong> (<code>/files/*</code>) — thumbnail variants (<code>?size=thumb</code>) are public for login-page previews. Web-sized variants and original files require authentication; video playback supports HTTP range requests for seeking.</li>
...
<ol>
    ...
    <li>Check PHP syntax with <code>php -l &lt;file&gt;</code> on each changed file before deploying.</li>
    <li>Deploy changed files to the server with <code>scp</code> and <code>ssh</code>, then copy them into the install directory (see <code>install.sh</code>).</li>
    <li>Storage files (<code>storage/theme.json</code>, <code>storage/admin-theme.json</code>, <code>storage/site-layout.json</code>, <code>storage/admin-layout.json</code>, <code>storage/themes/*.json</code>, <code>storage/uploads/*</code>) must be owned by <code>www-data:www-data</code>. After creating or replacing them: <code>sudo chown www-data:www-data /var/www/gallery/storage/&lt;file&gt;</code></li>
    <li>If PHP opcache is enabled and changes aren't reflecting, reset it: <code>php -r 'opcache_reset();'</code></li>
...
<ul>
    ...
    <li><strong>Three variants per image</strong> — the <em>original full-size file</em> (<code>/files/&lt;name&gt;</code>, kept byte-for-byte as uploaded), a <em>fast-loading web variant</em> (<code>web_&lt;name&gt;</code>, scaled to fit <code>uploads.web_max_width</code> = 1600px, generated by <code>create_web_image()</code>), and a <em>thumbnail</em> (<code>thumb_&lt;name&gt;</code>, 400&times;300, <code>?size=thumb</code>). <code>file_url($name, $size)</code> builds the URL: <code>''</code> = original, <code>'thumb'</code>, <code>'web'</code>. Deleting an orphan photo removes all three files (<code>Photo::deleteIfOrphan()</code>).</li>
    ...
    <li><strong>Upload limit</strong> — each file may be up to <strong>10 GiB</strong>. The application limit is in <code>config/app.php</code>; Apache/PHP limits are set in <code>public/.htaccess</code> as <code>upload_max_filesize=10G</code> and <code>post_max_size=11G</code>. Large uploads also require sufficient free space in <code>storage/uploads/</code>.</li>
